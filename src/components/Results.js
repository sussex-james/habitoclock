...
const Results = ({ results }) => {
    ...
    let tableDataOptimal = [];
    results.map((item) => {
        tableData.push({
            name: "  (optimal) " + item.name + " " + item.emoji,
            time: item.bestTime,
            carbon:
                (item.usage * item.watts) / 1000 -
                ((item.usage * item.watts) / 1000) * item.percentageDifference,
        });
    });
